 ...
 * coloured pane recipes. 26.1 resolves those previews from synced {@link SlotDisplay}s in the screen's extract
 * pass (1.21.4 named these renderRecipes / renderTooltip), and they cannot see the live input
 * (and carry no recipe object on servers), so this wraps the resolve and
 ...
 * old getResultItem call sites, and again when 26.1 renamed render* to extract*). The stonecutter
 * preview is a standing cell on the per-port
 * in-game checklist for exactly that reason.</p>
 */
@Mixin(StonecutterScreen.class)
public abstract class StonecutterScreenMixin {
    @WrapOperation(
            method = {"extractRecipes", "extractTooltip"},
            at = @At(value = "INVOKE",
