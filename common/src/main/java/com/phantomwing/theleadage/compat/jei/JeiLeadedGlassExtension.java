...
public class JeiLeadedGlassExtension implements ICraftingCategoryExtension<LeadedGlassDisplayRecipe> {
    ...
    /**
     * 1.21.4 turned recipe contents into {@link SlotDisplay}s, and JEI 21.4 made this the extension's
     * one required method: it is how JEI indexes the recipe for ingredient lookups, while the visible
     * layout still comes from {@link #setRecipe}. Cells that cycle report every colour variant as a
     * composite, so a lookup on any dyed pane finds this recipe.
     */
    @Override
    public List<SlotDisplay> getIngredients(RecipeHolder<LeadedGlassDisplayRecipe> recipeHolder) {
        List<SlotDisplay> displays = new ArrayList<>();
        for (List<ItemStack> variants : recipeHolder.value().inputVariants()) {
            if (variants == null) {
